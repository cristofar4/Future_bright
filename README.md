...
`DATABASE_URL_UNPOOLED` and `POSTGRES_URL_NON_POOLING` are accepted too. Vercel also
prefixes an integration's variables when the plain name is already taken, so any variable
*named* `…DATABASE_URL` or `…POSTGRES_URL` (for example `NEON_DATABASE_URL`) is considered
as well. By name only: nothing else in the environment is read, whatever its value, and
`POSTGRES_URL_NO_SSL` is skipped rather than reaching a hosted database with TLS off.

A **usable** URL wins over the list order: an integration can leave two set at once, and
taking `DATABASE_URL` just because it is first would refuse a working database sitting in
the next variable along. Only when none is usable does it fall back to the first one set,
so the setup page can name what it had to reject. `setup.html` says which variable it is
reading, so a database connected through Vercel's UI does not look like it was ignored.
The name only: never the string.
...
An administrator also gets **Pupils** and **Staff**: the register and the staff list, searchable,
filterable by class, each row opening that person's whole record. A pupil's record shows their
results, homework, attendance, guardian contacts and which portal accounts are linked to them;
a member of staff's shows the classes they take. Search terms and filters live in the page's
address, so a result can be linked to and the back button does what it looks like it does.
